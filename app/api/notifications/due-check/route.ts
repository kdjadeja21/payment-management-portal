...
export async function GET(request: NextRequest) {
  try {
    ...
    const retailerInvoices: { [key: string]: { 
      due: number; 
      overdue: number; 
      retailerName: string;
      invoices: Array<{id: string, dueDate: Date, invoiceNumber: string, userId: string, invoiceName: string}>;
    } } = {};
    ...
    for (const [retailerId, data] of Object.entries(retailerInvoices)) {
      ...
      if (data.invoices.length === 1) {
        ...
        if (isToday(invoice.dueDate)) {
          ...
          const message = `Your invoice <strong>${invoice.invoiceName}</strong> from <strong>${data.retailerName}</strong> is past due by <strong>${differenceInCalendarDays(today, invoice.dueDate)} day(s)</strong>.`;
          await addDoc(collection(db, 'notifications'), {
            userId: invoice.userId,
            invoiceId: invoice.id,
            type,
            title: 'Invoice Due Passed',
            message,
            createdAt: serverTimestamp(),
            read: false,
            link: `payment-management/invoices/${invoice.id}`,
      ...
      // Logic for multiple invoices
        ...
        if (hasDueToday && hasOverdue) {
          type = NotificationType.InvoiceOverdue;
          title = 'Invoices Due Today and Overdue';
          ...
          type = NotificationType.InvoiceDue;
          title = 'Multiple Invoices Due Today';
    ...
    return NextResponse.json({ success: true, message: `${notifiedRetailerIds.length} notifications created successfully.` });
  } catch (error) {
