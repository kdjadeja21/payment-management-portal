...
export async function addInvoice(
  invoiceData: Omit<
    Invoice,
    "id" | "status" | "dueDays" | "createdAt" | "updatedAt"
  >
): Promise<Invoice> {
  try {
    const { userId } = await auth();
    ...
    if (invoiceData.dueDate && invoiceData.dueDate !== currentData.dueDate) {
      ...
      if (dueDate.toDateString() === today.toDateString()) {
        await createNotification(
          ...
          `Your invoice <strong>${invoiceName}</strong> from <strong>${retailerName}</strong> is due today.`,
          invoiceLink
        );
        ...
        await createNotification(
          ...
          `Your invoice <strong>${invoiceName}</strong> from <strong>${retailerName}</strong> is past due by <strong>${daysPassed} day(s)</strong>.`,
          invoiceLink
        );
      }
    }
...
export async function deleteInvoice(invoiceId: string): Promise<void> {
  try {
    ...
    if (paymentSnapshot.docs.length > 0) {
      throw new Error(
        "Cannot delete invoice that is connected with payments. Please delete all related payments first."
      );
    }
